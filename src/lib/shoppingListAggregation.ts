...
export function aggregationKey(name: string, unit: string, amount: number | null): string {
  ...
  return `${normName}__${group ? 'weight-or-volume' : unit}`
}
...
export function toBaseAmount(item: RawShoppingItem): { amount: number | null; unit: string } {
  if (item.amount === null) return { amount: null, unit: item.unit }
  const group = unitGroup(item.unit)
  if (!group) return { amount: item.amount, unit: item.unit }
...
export function formatAggregatedAmount(amount: number | null, unit: string, lang: Lang): string {
  if (amount === null) return ''
  let value = amount
  let displayUnit = unit
  if (unit === 'g' && value >= 1000) { value /= 1000; displayUnit = 'kg' }
  else if (unit === 'ml' && value >= 1000) { value /= 1000; displayUnit = 'l' }
