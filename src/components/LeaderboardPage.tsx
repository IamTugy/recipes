    ...
    <div className="min-h-dvh bg-bg pt-20 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        <h1 className="font-serif text-2xl font-bold text-cream mb-2">
          {lang === 'he' ? 'טבלת דירוג' : 'Leaderboard'}
        </h1>
        <p className="text-sm text-cream/40 mb-6">
          {lang === 'he'
            ? 'צברו נקודות בכל פעולה באפליקציה - פרסום מתכון שווה הכי הרבה, אחריו בישול ודירוג, ולייק שווה קצת.'
            : 'Earn points for using the app - publishing a recipe is worth the most, cooking and reviewing next, a like a little.'}
        </p>

        {myPoints !== null && (
          <div className="card p-4 mb-6 flex items-center justify-between">
            <span className="text-sm text-cream/60">{lang === 'he' ? 'הנקודות שלי' : 'My points'}</span>
            <span className="font-serif text-xl font-bold text-amber">{myPoints}</span>
          </div>
        )}

        {loading ? (
          <div className="text-sm text-cream/40">{lang === 'he' ? 'טוען...' : 'Loading…'}</div>
        ) : entries.length === 0 ? (
          <div className="text-sm text-cream/40">
            {lang === 'he' ? 'אין עדיין פעילות לדירוג.' : 'No ranked activity yet.'}
          </div>
        ) : (
            ...
            {entries.map(entry => (
              ...
              >
                ...
                <span className="flex-1 truncate text-sm text-cream/80">
                  {entry.name ?? (lang === 'he' ? 'משתמש/ת' : 'A cook')}
                </span>
