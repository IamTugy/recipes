...
function drawFrame(ctx: CanvasRenderingContext2D, props: BackgroundCookStatusProps) {
  ...
  ctx.fillStyle = 'rgba(245, 238, 227, 0.5)'
  ctx.font = '600 15px system-ui, sans-serif'
  ctx.fillText(`${recipeTitle} · ${stepLabel}`, x, 20, CANVAS_W - 40)

  ctx.fillStyle = '#f5eee3'
  ctx.font = '600 22px system-ui, sans-serif'
  wrapText(ctx, stepText, x, 56, CANVAS_W - 40, 30, 4)

  if (nearestTimer) {
    ctx.fillStyle = 'rgba(216, 155, 78, 0.15)'
    ctx.fillRect(20, CANVAS_H - 70, CANVAS_W - 40, 50)
    ctx.textAlign = rtl ? 'right' : 'left'
    ctx.fillStyle = '#d89b4e'
    ctx.font = '700 24px system-ui, sans-serif'
    ctx.fillText(`⏱ ${formatSeconds(nearestTimer.remainingSeconds)}`, x, CANVAS_H - 58, 140)
    ctx.font = '400 14px system-ui, sans-serif'
    ctx.fillStyle = 'rgba(245, 238, 227, 0.6)'
    const labelX = rtl ? CANVAS_W - 170 : 170
    ctx.textAlign = rtl ? 'right' : 'left'
    ctx.fillText(nearestTimer.label, labelX, CANVAS_H - 55, CANVAS_W - 190)
  }
}
...
// Drives two ambient "guide mode is running in the background" surfaces on Android/Chrome:
...
const BackgroundCookStatus = forwardRef<BackgroundCookStatusHandle, BackgroundCookStatusProps>(
  function BackgroundCookStatus(props, ref) {
    const { active, recipeTitle, stepLabel, stepText, nearestTimer, lang } = props
