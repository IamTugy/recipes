...
describe('CollectionsController', () => {
  ...
  it("GET /collections returns the current user's collections", async () => {
    ...
    const controller = new CollectionsController(collectionsService as any, activityLog as any)
  ...
  it('POST /collections creates a collection for the current user', async () => {
    ...
    expect(activityLog.record).toHaveBeenCalledWith('user_1', undefined, 'collection_created', { name: 'Desserts' })
  ...
  it('DELETE /collections/:id removes the collection', async () => {
    ...
    expect(activityLog.record).toHaveBeenCalledWith('user_1', undefined, 'collection_deleted')
  ...
  it('POST /collections/:id/recipes adds a recipe to the collection', async () => {
    ...
    expect(activityLog.record).toHaveBeenCalledWith('user_1', 'a', 'recipe_added_to_collection')
  ...
  it('DELETE /collections/:id/recipes/:slug removes a recipe from the collection', async () => {
    ...
    expect(activityLog.record).toHaveBeenCalledWith('user_1', 'a', 'recipe_removed_from_collection')
